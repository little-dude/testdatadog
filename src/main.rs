...
#[tokio::main]
async fn main() {
    create_datadog_tracing_subscriber("http://localhost:8126");
    error!("error");
    warn!("warn");
    info!("info");
    debug!("debug");
...
fn create_datadog_tracing_subscriber(agent: &str) {
    ...
    // Never drop `uninstall` so that the pipeline is never removed
    let _: &'static _ = Box::leak(Box::new(uninstall));
